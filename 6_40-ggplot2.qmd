...
ggplot(data = ...,        # Call "economics" dataset
       aes(x = ...,       # Map "x" aesthetic
           y = ...)) +    # Map "y" aesthetic
  geom...() +             # Call correct geom
  theme_classic()         
...
Many ggplot2 color functions contain `brewer` in the name. That's because, by default, ggplot2 uses color palettes based on the research of [Dr. Cynthia Brewer](https://en.wikipedia.org/wiki/Cynthia_Brewer).

Dr. Brewer's premade palettes, all recognized in ggplot2, are available in [Color Brewer 2](http://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3).
...
# Replace the geom and add attributes
...
ggplot(midwest, 
       aes(x = popdensity, 
           y = percollege)) +
  geom_point(... = ...) +               # Set alpha
  geom...() +                           # Add smoother
  lims(x = ..., 
       y = ...) +                       # Set lims with c()
  scale...(... = number) +              # Specify function and argument
  labs(x = "...",
       y = "...",                       # Set labels in quotes
       title = "Density & Education") + 
  theme_minimal()
...
ggplot(data = diamonds,
  ...
  geom_point(alpha = 0.05, 
             size = 0.75) +
  labs(title = "Weight and selling price by cut",
       subtitle = "53,940 Diamonds",
       x = "Carat", 
       y = "Price") +
  scale_y_continuous(labels = dollar) +
  theme_minimal() + 
  ...(. ~ ...)        # Faceting won't work here; use R
...
## Package "ggplot2" Extensions

We used a "ggplot2" extension in the introduction to this chapter: ["GGally"](https://www.rdocumentation.org/packages/GGally/versions/1.4.0).

You can find a list of every single extension at the [**ggplot2 extensions gallery**](http://www.ggplot2-exts.org/gallery/).
...
To review the practice data from Hancock Airport, get the raw data URL [here](https://raw.githubusercontent.com/DS4PS/dp4ss-textbook/master/tables/hancock.csv).

There's a lot to master here - but it's worth it. These can help:

* [ggplot2 Tidyverse Page](https://ggplot2.tidyverse.org)
* [ggplot2 Cheat Sheet (RStudio)](https://github.com/rstudio/cheatsheets/blob/master/data-visualization-2.1.pdf)
* [ggplot2 Extensions Gallery](https://www.ggplot2-exts.org/gallery/)
* [The R Graph Gallery](https://www.r-graph-gallery.com/)
* [R Documentation: Scales](https://www.rdocumentation.org/packages/scales/versions/0.4.1)
* [R Graphing Library (Plotly)](https://plot.ly/r/)
* ["Visualization with ggplot2: Part I"](http://rpubs.com/JamisonCrawford/ggplot2)
* ["Visualization with ggplot2: Part II"](http://rpubs.com/JamisonCrawford/ggplot22)
* ["Visualization with ggplot2: Part III"](http://rpubs.com/JamisonCrawford/ggplot23)
